...
class CoPhyNet(nn.Module):
  ...
  def gcn_on_AB(self, struct_obs_ab):
    ...
    for i in range(T):
      ...
      x_1 = x.unsqueeze(0).repeat(K, 1, 1)  # (n+m, n+m, 56)
      # print("gcn_on_AB: x_1 shape ", x_1.shape)
      x_2 = x.unsqueeze(1).repeat(1, K, 1)  # (n+m, n+m, 56)
      # print("gcn_on_AB: x_2 shape ", x_2.shape)
      x_12 = torch.cat([x_1, x_2], -1).float() # (n+m, n+m, 2*(56))
      E = self.mlp_inter(x_12) # E (1, n+m, 32)
      # print("gcn_on_AB: E shape ", E.shape)

      # next position : o_t+1^1 = f(o_t^1,e_t+1)
      x_next_pos = x.unsqueeze(0) # x_next_pos (1, n+m, 56)
      # print("gcn_on_AB x_next_pos shape: ", x_next_pos.shape)
    ...
    out = torch.stack(list_out, 1) # (B,T,K,H) in our case B=1
  ...
  def pred_D(self, confounders, struct_obs_c, T=30):
    """

    :param stability: (B,K,1)
    :param confounders: (1, n+m, H) a.k.a (B,K,H)
    :param input: pose_cd= (1, n+m, 28+28) a.k.a (T,K,56)
    ...
    for i in range(T):
      ...
      # Cat
      x = torch.cat([pose, confounders], -1) #.detach() ??? # x (1, n+m, 56 + H)
      # print("pred_D: x shape ", x.shape)

      # compute interactions : e_t+1 = f(o_t^1,o_t^2,r)
      x_1 = x.unsqueeze(1).repeat(1, K, 1, 1)  # (1, n+m, n+m, 56+H)
      x_2 = x.unsqueeze(2).repeat(1, 1, K, 1)  # (1, n+m, n+m, 56+H)
      x_12 = torch.cat([x_1, x_2], -1).float() # (1, n+m, n+m, 2*(56+H))
      E = self.mlp_inter_delta(x_12) # (1, n+m, n+m, H)
      # print("pred_D: E shape ", E.shape)

      # next position : o_t+1^1 = f(o_t^1,e_t+1) with RNN on top
      _in = self.mlp_gcn_delta(torch.cat([x_1, E], -1).float()) # cat([x_1, E], -1)= shape(1, n+m, n+m, 2*(H)+56)
      # _in (1, 1, n+m, H)
      # print("pred_D: _in shape ", _in.shape)
      B = _in.size(0) # Breadth of network = 1
      list_new_hidden = []
      for k in range(K):
        if i == 0:
          ...
          hidden, *_ = self.rnn_delta(_in[:,:,k], list_last_hidden[k]) # (B,1,H)
          # hidden (B, 1, H)
          # print("pred_D: hidden shape ", hidden.shape)
  ...
  def forward(self, struct_obs_ab, struct_obs_c):
    ...
    # Run a GCN on AB
    seq_o = self.gcn_on_AB(struct_obs_ab) 
    ...
    # pred
    out = self.pred_D(confounders, struct_obs_c, T=T)
