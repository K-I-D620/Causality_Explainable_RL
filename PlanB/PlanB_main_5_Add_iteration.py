...
def Pretrain_CoPhy_model(device= None, env=None, RL_agent=None, initial_obs=None, num_of_objects=1, CF_model=None, print_freq=5, list_mse_3d=None, num_timesteps=30, max_iter=40):
...
def train_CoPhy_model(device= None, env=None, RL_agent=None, initial_obs=None, num_of_objects=1, CF_model=None, print_freq=5, list_mse_3d=None, num_timesteps=30, max_iter=40):
...
def main():
    ...
    task = generate_task(task_generator_id="stacking2", 
                        ...
                        dense_reward_weights=np.array([750,250,250,125,0.005]))
    env = CausalWorld(task=task, skip_frame = 3, enable_visualization=False)
    pretrained_RL_agent = Stacking2ActorPolicy()
    ...
    for _ in range(num_epochs_mass):
        ...
        for _ in range(num_epochs_goal):
            ...
            list_mse_3d, CF_causal_rep = Pretrain_CoPhy_model(device=device, env=env, RL_agent=pretrained_RL_agent, initial_obs=obs, 
    ...
    CausalRep_path = "./trained_CFRL_model_iter_stack"
    ...
    model_save_path = "./trained_CFRL_model_iter_stack"
    file_prefix = "0_CFRL"

    task = generate_task(task_generator_id="stacking2", variables_space='space_a',
                            dense_reward_weights=np.array([750,250,250,125,0.005]), mode=1, causal_rep=numpy_causal_rep,
    ...
    for curr_epoch in range(num_epochs):
        ...
        if curr_epoch >= 3:
            initial_obs = env.reset()
            list_mse_3d, CF_causal_rep = train_CoPhy_model(device=device, env=env, RL_agent=RL_model, initial_obs=initial_obs, 
                                                                num_of_objects=num_of_objects, CF_model=CF_model, print_freq=print_freq,
                                                                list_mse_3d=list_mse_3d, num_timesteps=num_timesteps, max_iter=max_iter)
            epoch_counter += 1
            with open(log_file_train, "+a") as f:
